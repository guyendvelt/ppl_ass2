...
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    if (!isArray(fields) || !allT(isString, fields)) {
        return makeFailure(`Invalid class fields: ${format(fields)}`);
    }
    if (!isGoodBindings(methods)) {
        return makeFailure(`Malformed class methods: ${format(methods)}`);
    }
    ...
    const bindingsMethods = mapv(methodValsResult, (vals : CExp[]) => zipWith(makeBinding, methodNames, vals))
    return mapv(bindingsMethods, (bindings : Binding[]) => makeClassExp(vars, bindings))    
}
